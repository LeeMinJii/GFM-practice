...
> 한 열의 셀 길이가 일치 할 필요가 없지만 일치하는 경우 읽기가 더 쉽다. 마찬가지로 선행 및 후행 파이프 사용도 일관되지 않을 수 있다.  
...
>다른 인라인 범위 내부를 포함해서 이스케이프 처리하여 셀 내용에 파이프를 포함한다.
...
>헤더 행 은 셀 수의 구분 기호 행 과 일치해야한다. 그렇지 않으면 테이블이 인식되지 않는다.
